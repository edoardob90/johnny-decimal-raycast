...
The index is a machine-managed JSON file (`.jdex.json`) stored in your root folder. Each entry maps a JD key to its type, name, parent key, and an optional description:
...
Descriptions survive a **Rebuild Index** — the command merges them back into the new index. To add or edit a description, use the **Edit Description** action (⌘E by default) on any search result.
